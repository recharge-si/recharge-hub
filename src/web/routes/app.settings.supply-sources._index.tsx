 ...
 * live on "Sync defaults" at the top. A location inherits them unless it says
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const mark = String(formData.get("warehouse") ?? "").trim();

  if (!locationId) {
    return fail("location", "Choose a Shopify location.");
  }
  ...
  const withLocationId = (result: ActionResult): ActionResult => ({
    ...result,
    locationId,
  });
  ...
  if (!mark) {
    ...
    return {
      ...
      locationId,
      message: "Disconnected the location.",
    };
  }

  const warehouse = warehouses.find((w) => w.mark === mark) ?? null;
  if (!warehouse) {
  ...
  /*
   * Connecting and editing want opposite things from a location that is
   * already taken.
   *
   * Editing location L to use warehouse W is a request to change what L uses,
   * so whatever L had must let go. Connecting W from the unconnected list is
   * not: nobody asking to connect a warehouse is asking to disconnect another
   * one, and doing it quietly is how a merchant loses a mapping they never
   * touched. The dialog only offers free locations, and this is the same rule
   * held on the server, where a stale form cannot get around it.
   */
  if (
    String(formData.get("via") ?? "") === "connect" &&
    atLocation &&
    atLocation.id !== forWarehouse?.id
  ) {
    return withLocationId(
      fail(
        "location",
        `${atLocation.name} already uses that location. Edit the location to change its warehouse.`,
      ),
    );
  }

  // Advanced: both fields fall back to the shop default unless overridden.
  const directionRaw = String(formData.get("direction") ?? INHERIT);
  ...
  const profitCenterRaw = String(formData.get("profitCenter") ?? INHERIT);
  ...
  if (direction === "mk_to_shopify") {
    const live = new Set(warehouses.map((w) => w.mark));
    ...
    if (clash) {
      return withLocationId(
        fail(
          "location",
          `${clash.name} already writes stock to this location. Turn its sync off first.`,
        ),
      );
    }
  }
  ...
  await appendEvent(principal, {
    ...
    detail: {
      ...
      location: String(formData.get("locationName") ?? "") || locationId,
      direction,
    ...
    <s-page heading="Locations">
      ...
      <s-modal id={HELP_MODAL_ID} heading="About locations">
        <s-stack direction="block" gap="base">
          ...
          <s-paragraph>
            Sync defaults answer where stock is counted. Each location follows
            them unless you change it.
          </s-paragraph>
          <s-paragraph>
      ...
      <s-stack direction="block" gap="large">
        ...
        <s-section heading="Sync defaults">
          <s-stack direction="block" gap="large">
            <s-text color="subdued">
            ...
            <Dropdown
              ...
              options={[
                ...
                { value: "none", label: "Do not sync stock" },
              ]}
              onChange={(next) =>
